...
\begin{verbatim}
$\lip |\mystyle{\v}| {List, Of, Names}$
\end{verbatim}

\section{The \texttt{\textbackslash lip} command}

In its simplest form, \texttt{\textbackslash lip} takes a list of
semicolon-separated lists of comma-separated values. Basically this just means
that semicolons are replaced with dots.
...
values. The command has to be given just before the values forming the
ellipsis, between two vertical bars. This command should use the \verb!\v! as
the value being stylized. Note that \verb!\v! is an alias for \verb!\#\#1!, but
the latter should not be used as the interface may evolve and not accept this
form anymore.

\example{$\lip |A_\v| {1, 2; n}$}

\example{$\lip |X| {1, 2, 3}$}

\section{The options}

The following options may be set globally using the \texttt{\textbackslash
lipsetdefault} command, or locally within the \texttt{\textbackslash lip}
...
\subsection{lip (= \texttt{\dots})}

Sets the ellipsis command(s).

\example{$\lip[lip=\cdots] {a, b; c, d}$}

\subsection{startlip and endlip (= false)}

Makes the sequence start or/and end with an ellipsis.

\example{$\lip[startlip] {n, n+1}$}

\example{$\lip[endlip] {1, 2}$}

\example{$\lip[startlip, endlip, lip=\cdots] {m, m+1; n-1, n, n+1}$}
